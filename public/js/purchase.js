...
if ($("div#import_invoice_dz").length) {
	$("div#import_invoice_dz").dropzone({
		url: base_path + '/import-purchase-invoice',
		paramName: 'file',
		autoProcessQueue: false,
		addRemoveLinks: true,
		uploadMultiple: false,
		...
		init: function () {
			...
			this.on("sending", function (file, xhr, formData) {
				...
				// when pressing 'import' button...
				$("#import_purchase_invoice").prop("disabled", true);
				$("#invoice-processor").show();
				$("#import_invoice_dz").addClass("darken-effect blur-effect");
			});
		},
		...
		success: function (file, response) {
			if (response.success) {
				...
				function renderTable() {
					...
					$.each(existingProducts, function (index, item) {
						...
						const columns = [
							...
							$('<td>').append(discountPercent),
							$('<td>').append(priceElement_01),
							$('<td>').append(lineTotal),
				...
				$("#new_item_add_btn_01").on('click', function () {
					const name = $("#add_invoice_item_01").val();
					const qty = $("#add_invoice_item_qty_01").val();
					const price = $("#add_invoice_item_price_01").val();
				...
				//Add products
				if ($('#search_product_to_add_01').length > 0) {
					$('#search_product_to_add_01')
						.autocomplete({
							source: function (request, response) {
								$.getJSON(
									'/purchases/get_products',
							...
							select: function (event, ui) {
								$(this).val(null);
								get_purchase_entry_row_01(ui.item.product_id, ui.item.variation_id);
							},
						})
						.autocomplete('instance')._renderItem = function (ul, item) {
							return $('<li>')
			...
			// after processing..
			$("#import_purchase_invoice").prop("disabled", false);
			$("#import_invoice_dz").removeClass("darken-effect blur-effect");

			this.removeAllFiles();

			$('#import_purchase_invoice_confirm').modal('hide');

			$("#invoice-processor").hide();
		},
	});
}

$("#invoice-processor").hide();
$("#item-adding-form").hide();
$("#import_purchase_invoice_confirm").hide();



$(document).on('click', '#import_purchase_invoice', function () {
	var invoicetDz = Dropzone.forElement("#import_invoice_dz");
	invoicetDz.processQueue();
})
